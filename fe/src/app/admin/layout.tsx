...
const menuGroups: MenuGroup[] = [
  ...
  {
    label: "Đơn hàng",
    items: [{ title: "Quản lý đơn hàng", url: "/admin/orders", icon: ShoppingCart }],
  },
  {
    label: "Người dùng",
    items: [
...
function AdminSidebar({ adminId }: { adminId: string | null }) {
  ...
  const isProductGroupActive =
    isPathActive("/admin/product") || isPathActive("/admin/category") || isPathActive("/admin/attribute");
        ...
        <SidebarMenuItem key={item.title}>
          <button
            ...
            className={`w-full flex items-center justify-between gap-3 px-5 py-3 transition-colors text-[13.5px] group ${
              isGroupActive ? "text-blue-600 font-semibold bg-blue-50/30" : "text-slate-600 hover:text-slate-900 hover:bg-slate-50"
            }`}
          >
            <div className="flex items-center gap-3.5">
              <item.icon className={`size-[18px] ${isGroupActive ? "text-blue-600" : "text-slate-400 group-hover:text-slate-600"}`} />
              <span>{item.title}</span>
            </div>
            <ChevronDown className={`size-3.5 transition-transform ${productMenuOpen ? "rotate-180" : ""}`} />
          </button>
          <div className={`${productMenuOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"} overflow-hidden transition-all`}>
                ...
                return (
                  <SidebarMenuButton
                    ...
                    className={`h-11 pl-12 pr-4 rounded-none border-r-[3px] ${
                      isSubActive ? "bg-blue-50/60 text-blue-600 font-bold border-blue-600" : "text-slate-500 hover:bg-slate-50 border-transparent"
                    }`}
                  >
                    <Link href={child.url} onClick={() => setOptimisticPathname(child.url)} className="flex items-center text-[13px]">
      ...
      <SidebarMenuItem key={item.title}>
        <SidebarMenuButton
          ...
          className={`h-11 rounded-none border-r-[3px] ${
            isActive ? "bg-blue-50 text-blue-600 font-bold border-blue-600" : "text-slate-600 hover:bg-slate-50 border-transparent"
          }`}
        >
          <Link href={item.url!} onClick={() => setOptimisticPathname(item.url!)} className="flex items-center gap-3.5 px-5 py-2 text-[13.5px]">
            <item.icon className={`size-[18px] ${isActive ? "text-blue-600" : "text-slate-400"}`} />
            <span>{item.title}</span>
          </Link>
        </SidebarMenuButton>
      </SidebarMenuItem>
    ...
    <Sidebar className="border-r border-slate-200 bg-white overflow-visible">
      <SidebarHeader className="h-16 border-b border-slate-100 bg-white p-0 overflow-visible">
        <div className="flex flex-row items-center justify-between h-full w-full px-5">
          <Link href="/admin/dashboard" className="flex items-center gap-2.5 hover:opacity-80">
            <div className="flex size-7.5 items-center justify-center rounded bg-slate-900 text-white shadow-sm">
              <TrendingUp className="size-4" />
            </div>
            <span className="font-bold text-slate-800 text-[14px]">PETNEST ADMIN</span>
          </Link>
      ...
      <SidebarContent className="py-2 custom-scrollbar">
        {menuGroups.map((group) => (
          <SidebarGroup key={group.label} className="p-0 mb-3 last:mb-0">
            <SidebarGroupLabel className="text-slate-400 uppercase text-[10px] font-bold px-5 py-2.5">{group.label}</SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu className="gap-0">{group.items.map(renderItem)}</SidebarMenu>
            </SidebarGroupContent>
          </SidebarGroup>
        ))}
      </SidebarContent>
      ...
      <SidebarFooter className="border-t border-slate-100 p-3 bg-white">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56 rounded-md shadow-xl border-slate-200 p-1" side="top" align="start">
            ...
            <DropdownMenuItem asChild className="rounded py-2 text-[13px] focus:bg-slate-50 cursor-pointer">
