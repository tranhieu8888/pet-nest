...
exports.getAllOrders = async (req, res) => {
  try {
    ...
    const orders = await Order.find({ userId })
      .select(
        "_id total status paymentMethod createAt OrderItems voucher reasonRejectCancel"
      )
      .sort({ createAt: -1 })
      .lean();
    ...
    const formattedOrders = orders.map((order) => {
      ...
      const orderVouchers = (order.voucher || [])
        .map((voucherId) => {
      ...
      OrderItems: order.OrderItems.map((orderItemId) => {
        ...
        if (orderItem) {
          ...
          const productVariant =
            productVariantMap[orderItem.productVariant?.toString()];
...
exports.getUserDashboard = async (req, res) => {
  try {
    ...
    const topUsersByCancellations = await Promise.all(
      topUsersByCancellationsAgg.map(async (user) => {
        // Find all cancelled order items for this user's cancelled orders
        const cancelledOrderItems = await OrderItem.find({
          _id: {
            $in: await Order.find({ _id: { $in: user.orderIds } }).distinct(
              "OrderItems"
            ),
          },
          status: "cancelled",
        ...
        const cancelRate =
          totalOrderCount > 0 ? user.cancelledOrderCount / totalOrderCount : 0;

        return {
          ...user,
          reasons: cancelledOrderItems
    ...
    const fullUserRegistrationByMonth = Array.from({ length: 12 }, (_, i) => {
      const monthData = userRegistrationByMonth.find(
        (item) => item._id === i + 1
      );
      return {
        month: i + 1,
    ...
    const fullUserRegistrationByMonthLastYear = Array.from(
      { length: 12 },
      (_, i) => {
        const monthData = userRegistrationByMonthLastYear.find(
          (item) => item._id === i + 1
        );
        return {
          month: i + 1,
    ...
    const fullUserRegistrationByMonthTwoYearsAgo = Array.from(
      { length: 12 },
      (_, i) => {
        const monthData = userRegistrationByMonthTwoYearsAgo.find(
          (item) => item._id === i + 1
        );
        return {
          month: i + 1,
    ...
    const result = {
      ...
      userRegistrationByMonthTwoYearsAgo:
        fullUserRegistrationByMonthTwoYearsAgo,
